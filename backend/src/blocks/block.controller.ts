...
export class BlockController {
    constructor(private blocksService: BlockService) {}

    @ApiOperation({summary: 'Создание блока'})
    @ApiResponse({status: 200, type: Block})
    @UsePipes(ValidationPipe)
    @Post()
    create(@Body() consultationDto: CreateBlockDto) {
        return this.blocksService.createBlock(consultationDto);
    }
    ...
    @ApiResponse({status: 200, type: Block})
    @ApiResponse({status: 404, description: 'Блок не найдена'})
